/**
 * Check-out screen — GPS + photos required to close out the rental.
 *
 * Both parties (renter returning item and host receiving it) must independently
 * verify GPS proximity and take ≥3 return photos. The screen calls
 * POST /api/v1/bookings/:id/check-out once all conditions are met.
 */
...
import GPSStatus from "../../../components/handoff/GPSStatus";
import PhotoGrid from "../../../components/handoff/PhotoGrid";
import AngleEnforcedCamera from "../../../components/camera/AngleEnforcedCamera";
...
export default function CheckOutScreen() {
  ...
  const handleComplete = async () => {
    ...
    setCompleting(true);
    try {
      await api.post(`api/v1/bookings/${transactionId}/check-out`, {
      ...
      Alert.alert("Check-out failed", "Could not complete check-out. Please try again.");
    } finally {
      setCompleting(false);
    }
  };
    ...
    <SafeAreaView className="flex-1 bg-white">
      ...
      >
        ...
        <View className="flex-row items-center gap-x-2 bg-green-50 rounded-xl px-3 py-2">
          ...
          <Text className="text-xs font-medium text-green-700">
            {isRenter ? "Returning the item" : "Receiving the item back"}
          </Text>
        </View>

        {/* Info notice */}
        <View className="bg-amber-50 rounded-xl px-3 py-2 flex-row items-start gap-x-2">
          <Ionicons name="information-circle-outline" size={16} color="#b45309" />
          <Text className="text-xs text-amber-700 flex-1">
            Both you and the {isRenter ? "host" : "renter"} must independently
            verify your location and take return photos before the rental closes.
          </Text>
        </View>
      ...
      <View className="px-4 py-4 border-t border-gray-100">
        ...
        >
          ...
          >
            {completing ? "Completing return…" : "Complete return"}
          </Text>
        </Pressable>

        {!proximity.canComplete && (
          <Text className="text-xs text-center text-gray-400 mt-2">
            Complete GPS verification + {MIN_PHOTOS} photos to continue
          </Text>
        )}
      </View>
    </SafeAreaView>
